...
这样是有问题的,统计结果可能出现误差.数据分布可能不均匀,比如某个分片里存在`money`前7名的用户,多出来的2个用户没排到前5,所以没在最后的结果里出现.这就导致最后结果少计算了2个用户.
...
如果我们的分片数量是5,想取金额前5名的数据,那么`shard_size`应该设置为5 * 5 = 25,才能保证不会出现误差的情况.
